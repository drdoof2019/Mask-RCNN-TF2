...
# train set
dataset_dir='renamed_to_numbers'
...
# Verisetinin train dosyasının içinden rasgele bir resim ve anotationlarını görmek için gerekli kod
import random
...
model.load_weights("weights/mask_rcnn_balloon.h5", by_name=True, exclude=["mrcnn_class_logits", "mrcnn_bbox_fc",  "mrcnn_bbox", "mrcnn_mask"])

# Add ModelCheckpoint callback to save weights after each epoch
checkpoint_callback = ModelCheckpoint(
    os.path.join(DEFAULT_LOGS_DIR, "mask_rcnn_epoch_{epoch:02d}.h5"),
    save_weights_only=True,
    verbose=1
)

# train weights (output layers or 'heads')
model.train(train_set, test_set, learning_rate=config.LEARNING_RATE, epochs=EPOCH_NUMBER, layers='heads', custom_callbacks=[checkpoint_callback])


"""
model_path = 'logs/mask_rcnn_fruits_cfg.h5'
model.keras_model.save_weights(model_path)
"""
###################################################

#BURAYA KADAR TRAIN ETTIK ASAGIDA DA OLUSTURDUĞUMUZ DOSYA İLE VERECEĞİMİZ İMAGEDEKİ VERİYİ TESPİT EDELİM.
...
# load model weights
model.load_weights('logs/mask_rcnn_epoch_20.h5', by_name=True)
...
fruit_img = skimage.io.imread("elma-agaci.jpg")
